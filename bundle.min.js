...
var Sudoku = (function (exports) {
    ...
    ***************************************************************************** */
    ...
    var areWeDoneYet = (function (s) { return s.matrix.every(function (row) { return row.every(function (val) { return Number.isInteger(val) && val > 0; }); }); });
    ...
    var option = function (coord, num) { return ({ coord: coord, num: num }); };
    ...
    var place = function (s) { return function (option) {
        var _a = __read(option.coord, 2), x = _a[0], y = _a[1];
    ...
    var iterateCoords = (function (s, group) {
        ...
        var collectionOfPossiVals = cg.coords.map(function (coord) {
            ...
            var currentSet = new Set(__spread(currentBlock.values, currentRow.values, currentCol.values));
            var r = s.allPossibleValues.filter(function (value) { return !currentSet.has(value); });
            console.log(r);
            return r;
    ...
    var iterateChanges = (function (s) {
        s.changes = __spread(new Set(s.changes.map(function (change) { return getUnique(s, iterateCoords(s, change)); }).reduce(function (arr, v) { return arr.concat(v); }, [])));
    ...
    var doku = (function (str) { return str.split('\n').slice(1, -1).map(function (row) { return row.split('').map(function (v) { return +v; }); }); });

    var s0 = doku("\n530070000\n600195000\n098000060\n800060003\n400803001\n700020006\n060000280\n000419005\n000080079\n");

    var makeCellGroupId = function (type, index) { return type + ":" + index; };
    var CellGroup = /** @class */ (function () {
        function CellGroup(identifier, coords) {
            return ({
                identifier: identifier,
                coords: coords,
                values: new Set(Array(coords.length).fill(0).map(function (v, i) { return i + 1; }))
            });
        }
        return CellGroup;
    }());
    var Sudoku = /** @class */ (function () {
        function Sudoku(matrix) {
            ...
            this.blocks = aryMaxVal.map(function (v, blockIdx) {
                ...
                var xOffset = (blockIdx % blockSize) * blockSize;
                var yOffset = (0 | blockIdx / blockSize) * blockSize;
                var block = Array(_this.maxValue).fill(0).map(function (_, cellIdx) { return [
            ...
            this.matrix = matrix;
            this.allPossibleValues = aryMaxVal.map(function (v, i) { return i + 1; });
            this.blocks.forEach(function (block) { return block.coords.forEach(function (coord) {
                block.values["delete"](matrix[coord[1]][coord[0]]);
            }); });
            this.rows.forEach(function (row) { return row.coords.forEach(function (coord) {
                row.values["delete"](matrix[coord[1]][coord[0]]);
            }); });
            this.cols.forEach(function (col) { return col.coords.forEach(function (coord) {
